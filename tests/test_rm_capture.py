"""Host-only smoke tests for scripts/rm-capture.py (no tablet needed)."""
...
CAP = importlib.util.spec_from_file_location(
    "rm_capture", "scripts/rm-capture.py")
rm_capture = importlib.util.module_from_spec(CAP)
CAP.loader.exec_module(rm_capture)
...
def test_write_png_is_a_complete_valid_png(tmp_path):
    ...
    rm_capture.write_png(str(out), w, h, stride, raw)
    blob = out.read_bytes()
...
def test_write_png_channel_order_bgr_to_rgb(tmp_path):
    out = tmp_path / "c.png"
    rm_capture.write_png(str(out), 1, 1, 4, bytes([30, 20, 10, 0]))
...
def test_rejected_capture_publishes_nothing(tmp_path, monkeypatch):
    ...
    with pytest.raises(rm_capture.Fail):
        rm_capture.capture(png, raw, True)
...
def test_preexisting_outputs_survive_rejection(tmp_path, monkeypatch):
    ...
    with pytest.raises(rm_capture.Fail):
        rm_capture.capture(str(png), str(raw), True)
...
def test_changed_starttime_aborts(tmp_path, monkeypatch):
    ...
    png, raw = str(tmp_path / "s.png"), str(tmp_path / "s.raw")
    with pytest.raises(rm_capture.Fail, match="identity changed"):
        rm_capture.capture(png, raw, True)
...
def test_short_raw_read_aborts_without_files(tmp_path, monkeypatch):
    ...
    png, raw = str(tmp_path / "s.png"), str(tmp_path / "s.raw")
    with pytest.raises(rm_capture.Fail, match="short raw read"):
        rm_capture.capture(png, raw, True)
...
def test_block_transport_error_uses_original_stderr_once(monkeypatch):
    ...
    cap = rm_capture.Cap()
    with pytest.raises(rm_capture.Fail, match="cannot skip"):
        rm_capture.read_block(cap, 1, [(0, 2 ** 32)], "hexdump", 4096, 4)
...
def test_short_block_read_reports_counts_without_reread(monkeypatch):
    ...
    monkeypatch.setattr("subprocess.run", fake)
    cap = rm_capture.Cap()
    with pytest.raises(rm_capture.Fail, match="short block read"):
        rm_capture.read_block(cap, 1, [(0, 2 ** 32)], "hexdump", 4096, 64)
    assert len(calls) == 1, "reread process memory for diagnostics"


def _snapshot_output(fw="20260827113527", size="22046100",
                     xohash=None, qthash=None):
    xohash = xohash or rm_capture.SUPPORTED_XOCHITL_SHA256
    qthash = qthash or rm_capture.SUPPORTED_QTGUI_SHA256
...
def test_snapshot_accepts_supported_build(monkeypatch):
    ...
    ident, maps, tool, regions, exe = rm_capture.snapshot(rm_capture.Cap())
    assert ident == (1234, 0)
    assert tool == "hexdump"
...
def test_snapshot_rejects_unknown_build(monkeypatch):
    ...
    with pytest.raises(rm_capture.Fail, match="unsupported build"):
        rm_capture.snapshot(rm_capture.Cap())


def test_output_aliasing_the_key_is_rejected(tmp_path):
    key = tmp_path / "id_test"
    key.write_bytes(b"k")
    rc = rm_capture.main(["--out", str(key), "--key", str(key), "--force"])
    assert rc == 2


def test_identical_outputs_are_rejected(tmp_path):
    same = str(tmp_path / "same.raw")
    assert rm_capture.main(["--out", same, "--raw", same]) == 2
...
def test_symlink_and_nonregular_outputs_are_rejected(tmp_path):
    ...
    assert rm_capture.main(["--out", str(link), "--force"]) == 2
    assert rm_capture.main(["--out", str(tmp_path)]) == 2
...
def test_publish_rejects_racing_writer_without_force(tmp_path, monkeypatch):
    ...
    with pytest.raises(rm_capture.Fail, match="exists"):
        rm_capture.publish(str(tmp), str(final), False)
...
def test_publish_no_force_and_force_paths(tmp_path):
    ...
    rm_capture.publish(str(src), str(tmp_path / "b"), False)
    assert (tmp_path / "b").read_bytes() == b"frame"
    assert not src.exists(), "staging link left behind"
    dst = tmp_path / "c"
    dst.write_bytes(b"old")
    with pytest.raises(rm_capture.Fail, match="exists"):
        rm_capture.publish(str(tmp_path / "b"), str(dst), False)
    assert dst.read_bytes() == b"old"
    rm_capture.publish(str(tmp_path / "b"), str(dst), True)
    assert dst.read_bytes() == b"frame"


def test_block_pipe_has_no_success_mask():
    pipe = rm_capture._block_pipe(1, 0, 1)
...
def test_block_pipe_reports_dd_failure_through_shell():
    pipe = rm_capture._block_pipe(1, 999999999, 1)
...
def test_block_error_reports_original_stderr_without_reread(monkeypatch):
    ...
    monkeypatch.setattr("subprocess.run", fake)
    with pytest.raises(rm_capture.Fail, match="SIMULATED I/O error"):
        rm_capture.read_block(rm_capture.Cap(), 1, [(0, 2 ** 32)],
                              "hexdump", 4096, 4)
    assert len(calls) == 1
...
def test_snapshot_rejects_same_size_different_binary(monkeypatch):
    ...
    monkeypatch.setattr("subprocess.run", _fake_run_factory(calls, out=bad))
    with pytest.raises(rm_capture.Fail, match="hash"):
        rm_capture.snapshot(rm_capture.Cap())
